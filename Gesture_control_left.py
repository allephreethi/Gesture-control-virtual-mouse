...
class Gest(IntEnum):
    # Binary Encoded
    """
    Enum for mapping all hand gestur aare to binary number.
    """
...
class Controller:
    ...
    def changesystembrightness():
        ...
        sbcontrol.set_brightness(current_brightness * 100, display=0)
    def changesystemvolume():
        ...
        # Adjust the volume based on the pinch gesture's magnitude
    ...
    def take_screenshot():
        ...
        if not Controller.screenshot_taken:  # Check if screenshot has not been taken yet
            try:
                screenshot = pyautogui.screenshot()
                screenshot.save(r"C:\Users\vamsh\OneDrive\Pictures\PREETHI\screen.png")  # Save the screenshot to a file
                print("Screenshot taken successfully!")
                Controller.screenshot_taken = True  # Set the flag to True after screenshot is taken
    ...
    def handle_controls(gesture, hand_result):  
        ...
        if gesture == Gest.V_GEST:
            ...
            # Add the ability to take a screenshot when the PINKY gesture is detected
...
class GestureController:
    """
    Handles camera, obtain landmarks from mediapipe, entry point for the whole program.

    Attributes
    ----------
    gc_mode : int
        Indicates whether gesture controller is running or not, 1 if running, otherwise 0.
    cap : Object
        Object obtained from cv2, for capturing video frame.
    CAM_HEIGHT : int
        Height in pixels of obtained frame from the camera.
    CAM_WIDTH : int
        Width in pixels of obtained frame from the camera.
    hr_major : Object of 'HandRecog'
        Object representing the major hand.
    hr_minor : Object of 'HandRecog'
        Object representing the minor hand.
    dom_hand : bool
        True if the right hand is the dominant hand, otherwise False. Default is True.
    """
    ...
    def classify_hands(self, results):
        ...
        the hand obtained from mediapipe, uses 'dom_hand' to decide major and
        minor hand.
        """
            ...
            while self.cap.isOpened() and self.gc_mode:
                ...
                if results.multi_hand_landmarks:
                    ...
                    if len(results.multi_hand_landmarks) == 1:
                        ...
                        if handminor.get_gesture() == Gest.PINCH_MINOR:
                            Controller.handle_controls(Gest.PINCH_MINOR, handminor.hand_result)
                        else:
                            Controller.handle_controls(handmajor.get_gesture(), handmajor.hand_result)

                        for hand_landmarks in results.multi_hand_landmarks:
                            mp_drawing.draw_landmarks(image, hand_landmarks, mp_hands.HAND_CONNECTIONS)
                    else:
                        # Ignore frame or take specific action for multiple hands
                        print("More than one hand detected. Ignoring frame.")
                else:
...
# Create and start the modified GestureController
